	...
	public static final void checkDonation(final Player player) {
		...
		GameEngine.slowExecutor.execute(new Runnable() {
			...
			public void run() {
				try {
					URL url = new URL(
							"http://matrixftw.com/donate/checkdonate.php?username="
					...
					if (noresult)
						...
						player.getPackets()
								.sendGameMessage(
										"<col=00ff00>Congratulations! You are now a "+(string == "1" ? "extreme " : "")+"donator.");
					}
				} catch (Throwable e) {
					player.getPackets()
							.sendGameMessage(
 ...
 * ("You've been promoted to donator till "+target.getDonatorTill()); } }
 * 
 * private Donations() {
 * 
 * } }
