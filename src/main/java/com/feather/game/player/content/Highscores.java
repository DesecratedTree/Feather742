...
public class Highscores {
				...
				try {
					synchronized(lock) {
						...
						URL url = new URL("http://www.matrixftw.com/updatehighscores.php?username="+
								...
								"&construction="+levels[21]+"&hunter="+levels[22]+"&summoning="+levels[23]+"&dungeoneering="+levels[24]+"" +
								"&totalxp="+totalxp+"&attackxp="+xp[0]+"&defencexp="+xp[1]+"&strengthxp="+xp[2]+"" +
								...
								"&constructionxp="+xp[21]+"&hunterxp="+xp[22]+"&summoningxp="+xp[23]+"&dungeoneeringxp="+xp[24]+(displayname != null ? "&oldname="+displayname+"" : "")+"");
	...
	private static boolean requirements(Player player) {
		for(int i = 0; i <= 24; i++) {
			if(player.getSkills().getXp(i) == 2.0E8) {
				return false;
			}
